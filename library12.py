...
st.session_state.selected_theme = st.sidebar.selectbox(
    "Choose a UI theme:", list(theme_options.keys())
)
...
# ✅ Retrieve your key from Streamlit Secrets
# In Streamlit Cloud: Settings → Secrets → add line:
# HUGGINGFACEHUB_API_TOKEN = "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"
from huggingface_hub import InferenceClient
token = os.getenv("HUGGINGFACEHUB_API_TOKEN")
client = InferenceClient("gpt2", token=token)  # ✅ Using GPT-2, which supports text-generation
...
with st.sidebar.expander("🧠 Ask the Library Mentor"):
    ...
    if st.button("Ask AI") and user_q:
        if client:
            try:
                response = client.text_generation(user_q, max_new_tokens=200, temperature=0.7)
                st.info(response.strip())
            except Exception as e:
                st.error(f"❌ Error reaching Hugging Face: {e}")
        else:
            st.error("🔑 API Key not found. Please set HUGGINGFACEHUB_API_TOKEN in Streamlit secrets.")
...
tabs = st.sidebar.radio("Choose Feature:", [
    "➕ Add Book", "📄 View Books", "✏️ Edit Book", "🗑️ Delete", 
    "📌 Reserve", "⭐ Rate", "🔍 Search", "📊 Stats", 
    "📤 Export", "📥 Import CSV"
])
...
if tabs == "➕ Add Book":
    ...
    if search_q:
        cursor.execute("""
            SELECT * FROM Library 
            WHERE BK_NAME LIKE ? OR BK_ID LIKE ? OR AUTHOR_NAME LIKE ?
